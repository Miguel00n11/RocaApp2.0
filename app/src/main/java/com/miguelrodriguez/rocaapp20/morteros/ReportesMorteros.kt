...
import com.miguelrodriguez.rocaapp20.morteros.ClaseObraMorteros
import com.miguelrodriguez.rocaapp20.morteros.ObraAdapterMorteros
...
class ReportesMorteros : AppCompatActivity() 
{
    ...
    companion object {
        lateinit var reporteSelecionado: ClaseObraMorteros
        var editar: Boolean = false
//        var listaObrasmutableListOf= mutableListOf<ClaseObraMorteros>()
    }


    private lateinit var btnRegistroMorteros: Button
    private lateinit var rvObrasMorteros: RecyclerView
    private lateinit var ObraAdapter: ObraAdapterMorteros

    private lateinit var swVerTodosReportesMorteros: Switch
    private lateinit var listaFiltrada: MutableList<ClaseObraMorteros>
    private lateinit var svBuscarReportesMorteros: SearchView
    
    //    private lateinit var claseObra: ClaseObra
    private lateinit var listaObrasmutableListOf: MutableList<ClaseObraMorteros>
    
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_reportes_morteros)

        initComponent()
        initUI()
    }
    private fun initComponent() {
        rvObrasMorteros = findViewById(R.id.rvObrasVMorteros)

        ReportesMorteros.reporteSelecionado = ClaseObraMorteros(
            1,
            "estacion",
            "a",
            "1",
...
//            "1",
        ...
        btnRegistroMorteros = findViewById(R.id.btnRegistroMorteros)
        ...
        svBuscarReportesMorteros = findViewById(R.id.svBuscarReportesMorteros)
        swVerTodosReportesMorteros = findViewById(R.id.swVerTodosReportesMorteros)
    }
    ...
    private fun initUI() {
        ...
        dataReference = FirebaseDatabase.getInstance().reference.child("Morteros").child("Reportes").child(personal)
        dataReferenceCampo = FirebaseDatabase.getInstance().reference.child("Morteros").child("Respaldo").child(personal)


        btnRegistroMorteros.setOnClickListener {
            val intent = Intent(this, RegistroMorteros::class.java)
            startActivity(intent)
        }


        rvObrasMorteros.adapter = ObraAdapterMorteros(listaObrasmutableListOf, onObraSelected = { position ->
            if (svBuscarReportesMorteros.visibility == View.GONE) {
                onItemSelected(position)
            }
        }, onItemDelete = { position -> onItemDelete(position) }, onVerReporteMorteros = { position ->
            onVerReporteMorteros(
                position, listaObrasmutableListOf
            )
        }, onSeleccionarNuevo = { position ->

            onItemSelected(position,true)
        }, mostrarBoton =  swVerTodosReportesMorteros.isChecked
        )

//        rvObrasMorteros.adapter = ObraAdapter
        ObraAdapter = rvObrasMorteros.adapter as ObraAdapterMorteros

        listaFiltrada = listaObrasmutableListOf

        rvObrasMorteros.layoutManager = LinearLayoutManager(this)

        cargarObras(dataReference)


        svBuscarReportesMorteros.setOnQueryTextListener(object : SearchView.OnQueryTextListener {
            ...
            override fun onQueryTextChange(fecha: String?): Boolean {
                ...
                if (searchText.isNotEmpty()) {
                    ...
                    } as MutableList<ClaseObraMorteros>
//
...
//                    )
                    ...
                    rvObrasMorteros.adapter = ObraAdapterMorteros(listaFiltrada,
                        { position -> /* código para manejar la selección de obra en la posición 'position' */ },
                        { position -> },
                        { position ->     onItemSelected(position,true,listaFiltrada) },
                        { position -> onVerReporteMorteros(position, listaFiltrada) },
                        swVerTodosReportesMorteros.isChecked

                    )

                } else {

                    rvObrasMorteros.adapter = ObraAdapterMorteros(listaObrasmutableListOf,
                        ...
                        { position ->
                            onItemSelected(position,true)},
                        { position -> onVerReporteMorteros(position, listaObrasmutableListOf) },
                        swVerTodosReportesMorteros.isChecked
        ...
        swVerTodosReportesMorteros.setOnCheckedChangeListener { buttonView, isChecked ->
            ObraAdapter.setMostrarBoton(swVerTodosReportesMorteros.isChecked)

            if (isChecked) {
                svBuscarReportesMorteros.visibility = View.VISIBLE// El switch está activado
                dataReference = FirebaseDatabase.getInstance().reference.child("Morteros").child("Respaldo").child(personal)
                cargarObras(dataReference)

            } else {
                svBuscarReportesMorteros.setQuery("", false)
                svBuscarReportesMorteros.visibility = View.GONE
                dataReference = FirebaseDatabase.getInstance().reference.child("Morteros").child("Reportes").child(personal)
                cargarObras(dataReference)
...
//                            placa.toString(),
    ...
    private fun onItemSelected(position: Int,clon:Boolean=false,listaReportes: MutableList<ClaseObraMorteros> =listaObrasmutableListOf) {
//        Toast.makeText(this, position.toString(), Toast.LENGTH_SHORT).show()

//        listaCalasmutableListOf[position], position


        ReportesMorteros.editar = true
        ReportesMorteros.reporteSelecionado = listaReportes[position]

        if (clon)
        {
            ReportesMorteros.reporteSelecionado.llave=dataReference.push().key.toString()
...
//        intent.putExtra("ReporteSeleccionado",listaObrasmutableListOf[position])
    ...
    private fun onVerReporteMorteros(position: Int, listaReportes: MutableList<ClaseObraMorteros>) = try {
        ...
        val registros = listOf(
            arrayOf("Cliente:", ReportesMorteros.reporteSelecionado.Cliente),
            arrayOf("Obra:", ReportesMorteros.reporteSelecionado.Obra),
            arrayOf("Loc.:", ReportesMorteros.reporteSelecionado.localizacion)
        )
...
//        val nombreArchivo = "registro_concreto_$fechaActual.pdf"
        ...
        val archivoPDF = File(directorio, "registro_mortero_$fechaActual.pdf")
        val alturaTexto = 8f
...
//            Toast.makeText(this, directorio.toString(), Toast.LENGTH_SHORT).show()
        try {
            ...
            if (ReportesMorteros.reporteSelecionado.tipoMuestreo=="Mezcladora estacionarias (fijas y basculantes)"){
            ...
            if (ReportesMorteros.reporteSelecionado.tipoMuestreo=="Olla de camión mezclador o agitador"){
            ...
            if (ReportesMorteros.reporteSelecionado.tipoMuestreo=="Camión caja con o sin agitadores"){
...
//
            ...
            if (ReportesMorteros.reporteSelecionado.tipoMuestreo=="Camión de volteo"){
...
//
            ...
            if (ReportesMorteros.reporteSelecionado.tipoMuestreo=="Pavimentadora"){
...
//
            ...
            if (ReportesMorteros.reporteSelecionado.tipoMuestreo=="Otro"){
                seleccionOtroTxt="/"
            }
            ...
            val nombreObraTxt = Cell(1, 3).add(Paragraph(ReportesMorteros.reporteSelecionado.Obra))
            ...
            val clienteTxt = Cell(1, 1).add(Paragraph(ReportesMorteros.reporteSelecionado.Cliente))
            ...
            val fechaTxt = Cell(1, 1).add(Paragraph(ReportesMorteros.reporteSelecionado.fecha))
            ...
            val localizacionTxt = Cell(1, 1).add(Paragraph(ReportesMorteros.reporteSelecionado.localizacion))
            ...
            val elemetoColadoTxt = Cell(1, 1).add(Paragraph(ReportesMorteros.reporteSelecionado.elementoColado))
            ...
            val ubicacionTxt = Cell(1, 1).add(Paragraph(ReportesMorteros.reporteSelecionado.ubicacion))
            ...
            val concreteraTxt = Cell(1, 1).
            add(Paragraph(ReportesMorteros.reporteSelecionado.concretera))
            ...
            val temperaturatxt = Cell(4, 1).
            add(Paragraph(ReportesMorteros.reporteSelecionado.temperatura.toString())).
            ...
            val moldeTxt = Cell(1, 1).
            add(Paragraph(ReportesMorteros.reporteSelecionado.Molde1.toString()))
            ...
            val revDis = Cell(1, 1).add(Paragraph("---"))
            ...
            val molde2Txt = Cell(1, 1).
            add(Paragraph(ReportesMorteros.reporteSelecionado.Molde2.toString()))
            ...
            val molde3Txt = Cell(1, 1).
            add(Paragraph(ReportesMorteros.reporteSelecionado.Molde3.toString()))
            ...
            val revObtenido = Cell(1, 1).add(Paragraph("---"))
...
//            val molde4Txt = Cell(1, 1).add(Paragraph(ReportesMorteros.reporteSelecionado.Molde4.toString()))
            ...
            val horarioSalida = Cell(1, 2).add(Paragraph("Salida de planta:  "+ ReportesMorteros.reporteSelecionado.horaSalida+" h" ))
            ...
            val horarioLLegada = Cell(1, 2).add(Paragraph("Llegada a obra:  "+ ReportesMorteros.reporteSelecionado.horaLLegada +" h"))
            ...
            val horarioMuestreo = Cell(1, 2).add(Paragraph("Inicio muestreo:  "+ ReportesMorteros.reporteSelecionado.horaMuestreo +" h"))
            ...
            val carretilla = Cell(1, 1).add(Paragraph("Carretilla:  "+ ReportesMorteros.reporteSelecionado.carretilla))
            ...
            val varilla = Cell(1, 1).add(Paragraph(" "))
            ...
            val mazo = Cell(1, 1).add(Paragraph("Mazo de hule:  "+ ReportesMorteros.reporteSelecionado.mazo))
            ...
            val flexometro = Cell(1, 1).add(Paragraph("Flexómetro:  "+ ReportesMorteros.reporteSelecionado.flexometro))
            ...
            val enrasador = Cell(1, 1).add(Paragraph("Enrasador:  "+ ReportesMorteros.reporteSelecionado.enrasador))
            ...
            val elaboradoTxt = Cell(1, 3).add(Paragraph(ReportesMorteros.reporteSelecionado.personal))
